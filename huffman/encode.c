...
void pq_insert(struct tnode* p)
{
   ...
   else /*insert between prev and next*/
   {
		...
		/* p->freq is not < qhead->freq
 		   set curr = qhead anyway. then itterate over 
 		   the linkedlist until a freq val is lower
		   otherwise put value at the end of the priority 
		   queue
